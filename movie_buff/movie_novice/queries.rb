...
def star_wars
  ...
  # hint: use 'select' and 'where'
  Movie.select(:id, :title, :yr).where("title LIKE 'Star Wars%'").map { |movie| [movie.id, movie.title, movie.yr] }
end
...
def below_average_years
  ...
  Movie.select(:yr, "COUNT(score < 5) AS bad_movies").where("score < 5").group(:yr).order(yr: :desc).map{ |movie| [movie.yr, movie.bad_movies] }
end
